...
| `service` | `domain`, `port`, `configs`, `komon` | `adapter`, DB drivers, HTTP |
| `adapter/inbound` | `port/inbound`, `domain`, `komon` | `adapter/outbound` directly |
...
Rules:
- **One domain struct per file** — each file in `internal/core/domain/` owns one entity and its related types (e.g. `domain/widget.go` contains `Widget`, `WidgetFilter`, and widget-specific constants/statuses). Do not put multiple unrelated entities in one file.
...
- [ ] One domain struct (entity) per file in `internal/core/domain/`
- [ ] Domain has no external dependencies, only `qwery` tags
