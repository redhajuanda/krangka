...
krangka uses `komon/pagination` with `qwery` for **offset** and **cursor** pagination. The same code path works for both — qwery generates SQL automatically based on `pagination_type`.
...
4. **Repository**: Use `.WithPagination(pagination).WithOrderBy(...)` — qwery adds LIMIT/OFFSET or cursor logic
