...
type UserFilter struct {
    Search *string `qwery:"search"` // optional filters use *bool or *string
}
```

Rules: plain structs, `qwery` tags only, no JSON tags, include `DeletedAt int`.
...
// Add to constructor
UserRepository: implRepo.NewUserRepository(qwery.Client),
...
**CRITICAL:** Do **NOT** add `UserRepository` (or any repository field) to the `&mariaDBRepository{ ... }` struct that is created **inside** `DoInTransaction`. That transaction registry must contain only `cfg`, `log`, `qwery`, `qweryTx`, `outbox`. The getter already returns a tx-backed instance when `qweryTx != nil`.
