...
- If handler needs a service, inject it the same way as HTTP handlers but use `GetQweryWorker()`
...
| Handler with service | Use `GetQweryWorker()` + `GetRepository` + `GetServiceXxx(repo)` |
