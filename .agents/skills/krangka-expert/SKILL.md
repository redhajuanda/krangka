...
| Writing or modifying a repository, migration, SQL query, transaction (`DoInTransaction`), or using the qwery SDK | [repository.md](references/repository.md) |
...
- **Workers use `GetQweryWorker()`** — never `GetQweryMain()`
- **Inside `DoInTransaction` use `repo` (lambda arg)** — never `s.repo`
- **Cache APIs follow current outbound cache contract** — do not reintroduce deprecated names/signatures
- **Dlock TTL is `time.Duration`** — never `int`
- **HTTP CORS must use `http.allow_origins` + `middleware.CORS(cfg.Http.AllowOrigins)`**
- **Keep HTTP request logger middleware enabled** (`middleware.LoggerMiddleware`)
- **Superadmin checks must use role name** (`claims.RoleName` + `roles.superadmin`)
- **Redisstream adapters stay split** (`redis.go`, `publisher.go`, `subscriber.go`) — no monolithic `conn.go`
- **Kafka adapters stay split** (`publisher.go`, `subscriber.go`)
- **Warehouse integration uses outbound `Warehouse` port** (Snowflake-backed `ListSchemas`)
