...
2. **Find out why it failed** — capture stderr and exit code. Common causes:
   - **Network/sandbox:** `no secure protocol found for repository` — Go cannot fetch private GitLab deps (`qwery`, `komon`). Run with `network` or `full_network` permission, or ask the user to run the gather locally.
